...
CREATE TABLE commands (hostname VARCHAR(40), operation enum('RUN','DOWNLOAD','UPLOAD','QUIT'), command VARCHAR(1024), command_id INT(11) NOT NULL AUTO_INCREMENT, file_position INT DEFAULT 0, filesize BIGINT DEFAULT 0, client_file_path VARCHAR(1024), server_file_path VARCHAR(1024), status enum('PENDING','COMPLETE','UNKNOWN','ERROR','PREPARING', 'STARTED'), log_file_path VARCHAR(1024), analyst_file_path VARCHAR(1024), PRIMARY KEY (command_id));
